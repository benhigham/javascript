...
- `browser.json`: raise `target` `ES2015` → `ES2022` and add `DOM.Iterable` to `lib` (`["DOM", "DOM.Iterable", "ESNext"]`). The browser configs are `noEmit` with a real bundler downstream, so the TS target should sit high and let the bundler downlevel per browserslist. This also fixes the inheritance chain (`next.json → react.json → browser.json`) silently resolving the effective `target` to `ES2015`.
